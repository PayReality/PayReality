...
export const LEARN_ARTICLES: LearnArticle[] = [
  ...
  },
  {
    ...
    body: "PayReality involves a human reviewer in two genuinely different situations, and it's easy to confuse them because both get called \"review\" or \"approval\" in everyday language. The first is decision-level Human Review: an AI agent's real action was evaluated, and Runtime Authority couldn't clearly match it to an Allow or a Deny, so it was sent to a person to resolve rather than the system guessing. That resolved decision (approved or rejected, with a reason) becomes part of that decision's own Evidence, permanently. The second is policy-level review and approval: before a Runtime Policy is ever published and starts governing real actions, it goes through its own separate authoring workflow, drafted, submitted for review, and approved or rejected, entirely independent of any specific agent action. A policy being \"in review\" has nothing to do with any particular decision being stuck; it means a rule itself hasn't been published yet. If you're looking for decisions waiting on a person to resolve, that's the Pending Review Queue. If you're looking for policy drafts waiting on approval before they can be published, that's inside Governance, on the policy's own page.",
  },
  {
