...
- That a downstream external action actually executed. Neither an ALLOW Decision, a consumed Capability Authorization (agent-direct path only — see below), nor an Authorization Receipt is proof of execution; all three prove an authorization decision was made, never that whatever it authorized actually happened afterward.
...
- That a Capability Authorization token, if issued and consumed, means anything happened downstream — a token is a transport and proof mechanism only, and one is never issued at all for an Adapter-mediated decision (a deliberate, current scope boundary, not an oversight).
