...
| POST | `/capability-tokens/verify` | 🛡️`CAPABILITY_VERIFY` (Trusted Integration Phase 6.1: previously Operator-key-only via `verify_operator_key` directly, not tenant-scoped at all; now `require_permission` + `get_current_organization`, the same pattern every other org-scoped Capability endpoint already uses. The Operator Key still works, provided it names its target organisation via `X-PayReality-Organization-Id` like every other operator-key call since Milestone 2) | Verifies and atomically consumes a Capability. Optional `environment`/`enforcement_binding_id` request fields (Phase 5) pin an expectation against the token's own signed claim if supplied; omitted, they're skipped. `409 capability_binding_mismatch` on a mismatch, alongside the pre-existing `404`/`401`/`403`/`409` outcomes for not-found, expired, audience-mismatch, constraint-mismatch, invalid-signature, and already-consumed. Phase 6.1: `403 capability_tenant_mismatch` if the token's own signed organisation doesn't match the caller's; `409 origin_agent_not_active` / `409 integration_identity_not_active` / `409 enforcement_binding_not_active` / `409 tenant_not_active` if a live-status freshness recheck at consumption time fails. |
