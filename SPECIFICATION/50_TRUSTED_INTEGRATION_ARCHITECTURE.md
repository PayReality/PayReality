...
No new issuance path was added for post-review decisions, on either runtime path. A Capability can only ever be issued for a Decision whose `outcome` is literally `ALLOW`. A `HUMAN_REVIEW` decision that a human later approves still has `outcome == HUMAN_REVIEW` forever (§50.15), and remains permanently ineligible: the existing `DecisionNotAllowError` check inside `issue_capability_for_decision` is unchanged and applies identically to both paths.

**What this does not change**: the existing operation-idempotency mechanism (§50.7). A retried business operation still resolves to the same, single Decision; issuing a Capability against that one Decision, however many times the underlying operation was retried, is still issuing it against one Decision, never a second authority. Nor does this change the underlying enforcement boundary described in §50.10 and §50.14: a Capability being issued or consumed is still never proof the downstream external action executed, on either path. The Capability itself remains, exactly as before, not itself enforcement: nothing in this codebase forces any downstream system to check it. §50.16 describes what a customer can now *declare* about their own checkpoint's requirement, and its explicit limits.
