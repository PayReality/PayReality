...
| Cross-tenant data exposure | **Milestone 1 (Security & Authorization Hardening) correction to this row's prior claim**: this was not actually N/A — Evidence read/verify, organisation-structure (business unit/department/team) CRUD, Principals, and all 14 Authority Graph read endpoints had no organization filter (several had no auth check at all), confirmed and closed this milestone. Every fixed endpoint now resolves the caller's organisation via `get_current_organization` and treats a different organisation's row identically to one that doesn't exist. 18 isolation regression tests added (`test_organization_isolation.py`), first of their kind in this codebase. | Two gaps remain, deliberately deferred, not fixed by this milestone: (1) the shared Operator Key still resolves to "whichever organisation was created first" with no per-org identity of its own — a genuine multi-tenant deployment cannot use it as-is; (2) `RuntimePolicyRecord` and the Runtime Policy Lifecycle tables have no `organization_id` at all, and the compiled `policies` table enforces exactly one active row **platform-wide** by database constraint — this is a schema-level, not query-level, gap, tracked as Milestone 2. Also not yet covered: the mutating Authority Builder endpoints (`resolve_principal`, `resolve_relationship`, `activate_relationship`, `answer_question`, `approve_graph`) don't yet verify their target row's organisation, only that the caller holds `AUTHORITY_REVIEW` — a related, smaller, explicitly noted-but-deferred gap. |
