...
**Phase 2A, small scoped backend additions (done):** `policy_version`/`policy_bundle_hash`/`authority_version`/`created_at` are now exposed directly on `GetDecisionResponse` (read server-side off the decision's earliest Evidence record, plus the real `Decision.created_at` column), so the Decision panel no longer needs a second fetch just to show them. Signer/certificate detail is now wired into the Evidence section from the already-existing `agentsApi.listCertificates` endpoint.

**Historical Policy Binding (done):** the one blocking gap Phase 2B needed first, the missing join between a decision's bundle (`Decision.policy_id`) and which `RuntimePolicyRecord`s that bundle actually contained, is closed. `Policy.bundle_manifest` (migration `ed6215ef0acc`) persists it, populated from a value `compile_bundle` already computed and previously discarded. `GET /v1/decisions/{id}/policy-binding` exposes it, org-scoped. Full detail in `HISTORICAL_POLICY_BINDING_IMPLEMENTATION.md`; proven sufficient for reconstruction (including feeding the existing Simulator explainer a historical decision's exact original policy state after two subsequent redeploys) in `HISTORICAL_POLICY_BINDING_TEST_REPORT.md`.

**Phase 2B, still not started, now unblocked:** run the existing Simulator explainer (`policy_simulation/explainer.py`) against a resolved decision's stored context, using the now-real Historical Policy Binding above, to populate real per-condition breakdown and a genuine Decision Explanation panel instead of omitting it. This is now a wiring task (build the `intent`/`context` reconstruction into a real function instead of test-only glue, then surface it in the UI), not a data-availability problem.
