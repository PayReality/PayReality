...
**Phase 2B, still not started:** run the existing Simulator explainer (`policy_simulation/explainer.py`) against a resolved decision's stored context to populate real per-condition breakdown and a genuine Decision Explanation panel instead of omitting it. See `RUNTIME_DECISION_CENTER_HISTORICAL_POLICY_BINDING_ANALYSIS.md`: the blocking gap this needs first is narrower than originally assumed here, one missing join (which `RuntimePolicyRecord`s a given compiled `Policy` bundle contained), not a broad architectural problem, since both the bundle identity (`Decision.policy_id`) and every policy's historical content (`RuntimePolicyRecord`, immutable per version) are already durably persisted today.
