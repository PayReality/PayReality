...
| Item | Customer value | Complexity | Risk | Dependency | Strengthens the story |
|---|---|---|---|---|---|
| Per-condition policy explainability on live decisions, reusing the Simulator's explainer | Very high, this is the single most-requested capability in the original brief | High | Medium (must not regress the Simulator's own use of the same explainer) | Was: solving the historical-policy-set persistence problem first. Now closed: see `HISTORICAL_POLICY_BINDING_IMPLEMENTATION.md`, `Policy.bundle_manifest`. Remaining work is wiring, not data availability. | Very high, this is the "why" a Decision Center is supposed to answer |
...
**Recommendation: the per-condition explainability item is the one worth real investment.** It's the most valuable single thing this audit found, reuses existing, already-correct logic rather than inventing a second engine, and is exactly what the original brief asked for. Its one blocking prerequisite, the historical-policy-set persistence problem (what a past Decision's bundle actually contained, so explaining it never silently substitutes today's active policy for the one actually evaluated), is now closed: see `HISTORICAL_POLICY_BINDING_IMPLEMENTATION.md` and `HISTORICAL_POLICY_BINDING_TEST_REPORT.md`, which prove the binding gives the existing explainer everything it needs, including after two subsequent redeploys. What remains for Phase 2B is turning that proof into a real code path (an `intent`/`context` reconstruction function, then UI wiring), still awaiting its own scoping and approval, not a further design or data-availability question.
