...
## 17. Product Experience V3.2 additions

The first milestone to actually apply this document to the shipped product surface (the earlier sections describe the un-applied V3 foundation). Locked top-level IA unchanged; every addition below is additive to the system above, not a replacement.

**Sidebar collapse.** `--pr-sidebar-width-expanded: 252px` / `--pr-sidebar-width-collapsed: 68px` (`theme.css`); the `<aside>` transitions `width` only, using the existing `--pr-motion-base`/`--pr-motion-ease` pair, so it inherits the same restraint as everything else in §12 rather than a bespoke easing curve. Collapsed state persists per-viewer in `localStorage` (`payreality_sidebar_collapsed`, mirroring `lib/theme.ts`'s own pattern) and is read synchronously before first paint, the same "no flash" discipline the theme toggle already established. Desktop-only: the mobile `Sheet` drawer (§13) is a structurally separate path and always renders fully expanded, unaffected by this preference. Collapsed nav items keep their icon and switch their label to a `Tooltip` (new dependency, `@radix-ui/react-tooltip`, keyboard-focus-triggered, not just hover) rather than disappearing, so the same information stays reachable, just denser.

**CTA contrast fix.** `--pr-authority-blue-cta: #3D5FD1`, a darker shade used only where white text fills directly onto authority blue (the primary `Button` variant, `DemoBanner`'s CTA) — `--pr-authority-blue` itself measured ~3.73:1 with white, short of WCAG AA's 4.5:1 for normal-size text. Every other use of `--pr-authority-blue` (borders, focus rings, links, the `AuthorityChain` active dot) is unaffected; those aren't text-on-fill contrast cases and didn't need the darker shade. `DemoBanner`'s own CTA pill switched from a translucent *white* overlay to a translucent *black* one (`rgba(0,0,0,0.18)`) for the same reason: lightening a light background pushed contrast the wrong direction.

**Body-copy-as-muted-text fix.** `--pr-text-muted` remains correct for genuinely secondary/caption text; the one place it was carrying a paragraph of primary explanatory prose (the demo landing hero) now uses `--pr-text-secondary` instead, an existing token, not a new one. Rule of thumb: if a `<p>` is the main readable content of its section rather than an annotation beside something more prominent, it needs `--pr-text-primary` or `--pr-text-secondary`, not `--pr-text-muted`.

**Page-transition motion.** `Layout.tsx`'s `<main>` replays `.pr-enter` on every route change (main content only; the sidebar next to it never moves). Implemented by toggling the class off, forcing a reflow, and re-adding it on `location.pathname` change — deliberately not a React `key` remount, which would tear down and rebuild whatever the route renders (resetting a page's own state even between two params of the same route) as a side effect of a purely visual change.

**Loading skeletons.** `Skeleton`/`SkeletonRows` (§7, pre-existing) now also cover Governance's rule editor (`PolicyWorkspacePage`), Agent Detail, Publish, and Simulate's own top-level loading states, replacing a plain "Loading..." text flash. Overview, the Governance list, Decision Detail, and Integration Detail already had skeleton coverage before this milestone.

**Draft with AI.** A secondary, clearly-labeled entry point (`Sparkles` icon, ghost button, never the primary action) beside Governance's "Save draft." Opens a `Sheet` panel, not a chat page: Draft / Edit / Explain modes, a plain-English change-preview before anything is applied, explicit Apply/Reject controls, "AI-proposed changes" labeling on the diff itself. Applying is a plain in-memory form-state merge (never a save, publish, or approve call) — see `PolicyWorkspacePage.tsx`'s `applyAiProposal` and `AI_ASSISTED_POLICY_DRAFTING.md` for the merge and safety contract. Uses the existing `EFFECT_LABEL`/`describePolicy` vocabulary, not a second copy of the outcome-language table in §6.

**Outcome-language wording.** The three effect radios in Governance's rule editor now read "Allow" / "Do not allow" / "Require human approval" (`describePolicy.ts`'s `EFFECT_LABEL`), describing what the *rule says to do*. The canonical customer-facing Decision outcome vocabulary in §6 ("Allowed" / "Not allowed" / "Needs human approval," describing what *happened*) is unchanged and still used everywhere a Decision's actual outcome is shown (`describePolicy.ts`'s separate `EFFECT_OUTCOME_LABEL`, used only inside the plain-English sentence itself).
